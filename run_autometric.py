...
learning_rate = 0.1
momentum = 0.1
logid = open('runLogAutometric.txt', 'w')
runOption = 1
##################################################################################
siamese = inference.siamese_autometric();
...
with tf.Session() as sess:
    ...
    if runOption == 1: ### starting from random weights
        ...
        saver.restore(sess, 'models/models_auto_95000.ckpt')
    for step in range(200000):
        ...
        sketch_x3s, sketch_y3s = dataset.next_batch(batch_train_size, 'sketch_train')
        sketch_x4s, sketch_y4s = dataset.next_batch(batch_train_size, 'sketch_train')
        ########## for shape ######################################
...
#############################################################################################
        ...
        if step % display_interval == 0 and step > 0:
            ...
            model_path = './models/models_auto_' + str(step)+'.ckpt'
...
############ for email #####################
    ...
    server.starttls()
    server.login('daiguoxian29@gmail.com', 'Dai29->Fool')
    server.sendmail('daiguoxian29@gmail.com', 'daiguoxian29@gmail.com', 'SHREC 14 Running (10000 iterations) run.py is finished')
    server.quit()
    logid.close()
